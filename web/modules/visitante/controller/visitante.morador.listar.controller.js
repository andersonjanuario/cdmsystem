(function () {
    ...
    .controller('VisitanteMoradorListarController', VisitanteMoradorListarController);

    VisitanteMoradorListarController.$inject = ['$scope', 'VisitanteMoradorFactory', 'MoradorService','$state'];


    function VisitanteMoradorListarController($scope, VisitanteMoradorFactory, MoradorService,$state) {
        //Atributos
        var vm = this;
        ...
        vm.pesquisa = '';
        vm.order = 'tb_cdm_visitante_morador.data_visita';
        vm.sort = false;
        ...
        objeto.isView = true;
        VisitanteFactory.setVisitante(objeto);
            $state.go('visitante-manter');              
        }    
...
/*
        function remover(codigo){
            VisitanteService.remove(codigo).then(function onSuccess(response) {
...
*/
        ...
        function findByFilter(skipIn, takeIn, pesquisa, order, sort) {
            MoradorService.findByDateMorador(skipIn, takeIn,pesquisa, order, sort).then(function onSuccess(response) {
                ...
                vm.moradores = VisitanteMoradorFactory.convertList(response.data);                 
            }).catch(function onError(response) {
                console.log(response);
            });
        };
